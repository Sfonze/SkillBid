...
async function main() {
  ...
  await sql`INSERT INTO sme_profiles (user_id, company_name, foundation_date, vat_number, core_business, tasks_completed_before, verified)
    VALUES (${greenfields.id}, 'Greenfields Logistics B.V.', '2014-03-01', 'NL123456789B01', 'Sustainable freight & last-mile logistics', 6, true)`;

  const [nova] = await sql`INSERT INTO users (email, password_hash, role) VALUES ('team@novastudio.nl', ${pw}, 'SME') RETURNING id`;
  await sql`INSERT INTO sme_profiles (user_id, company_name, foundation_date, vat_number, core_business, tasks_completed_before, verified)
    VALUES (${nova.id}, 'Nova Studio', '2019-08-12', 'NL987654321B02', 'Brand & digital design agency', 2, true)`;

  const [vellum] = await sql`INSERT INTO users (email, password_hash, role) VALUES ('contact@vellumlegal.eu', ${pw}, 'SME') RETURNING id`;
  await sql`INSERT INTO sme_profiles (user_id, company_name, foundation_date, vat_number, core_business, tasks_completed_before, verified)
    VALUES (${vellum.id}, 'Vellum Legal Partners', '2021-01-20', 'BE0123XYZ', 'Corporate legal advisory', 1, false)`;
  ...
  await sql`INSERT INTO student_profiles (user_id, full_name, university, university_email, languages, skills, completed_tasks_count, rating, verified)
  ...
  await sql`INSERT INTO messages (task_id, from_user_id, from_role, text, created_at) VALUES
    ...
    (${t4.id}, ${sam.id}, 'STUDENT', 'Just submitted them on the milestone — let me know your thoughts.', ${daysAgo(10)}),
    (${t4.id}, ${nova.id}, 'SME', 'These look great, approved! Starting on the build now?', ${daysAgo(9)}),
    (${t4.id}, ${sam.id}, 'STUDENT', 'Yep, started today — will share a staging link once responsive QA is done.', ${daysAgo(9)})
  `;
